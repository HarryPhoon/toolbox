...
# Helpful globals
DEFAULT_FEDORA_VERSION=${DEFAULT_FEDORA_VERSION:-"f30"}
REGISTRY_URL=${REGISTRY_URL:-"registry.fedoraproject.org"}
TOOLBOX_DEFAULT_IMAGE=${TOOLBOX_DEFAULT_IMAGE:-"registry.fedoraproject.org/f30/fedora-toolbox:30"}
TOOLBOX_TIMEOUT=${TOOLBOX_TIMEOUT:-60}
...
function setup_with_one_container() {
    ...
    # Create a new container if no other are present
    create_toolbox 1
}
...
function get_images() {
    ...
    for i in $(seq $numberof); do
        ...
        run-podman pull "$image" || echo "Podman couldn't pull the image."
    done
}
